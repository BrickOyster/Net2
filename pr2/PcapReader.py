    ...
    def get_80211_info(self, packet):
        ...
        try:
            ...
            info = {
                ...
                # Extra fields for project 2 claculations
                'timestamp': packet.sniff_time.timestamp() if hasattr(packet,'sniff_time') else None, # Timestamp of the packet (float,seconds)
                'length': int(packet.length) if hasattr(packet, 'length') else None, # Length of the packet in bytes
            }
            return info
